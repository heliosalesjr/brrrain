...
const STATUS_BADGE: Record<ConceptStatus, { label: string; variant: 'default' | 'info' | 'warning' | 'success' }> = {
  ...
  learning:  { label: 'Aprendendo',variant: 'info' },
  reviewing: { label: 'Revisando', variant: 'warning' },
...
export function Areas() {
  ...
  const [conceptModal, setConceptModal]   = useState<string | null>(null); // areaId
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Áreas de estudo</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            {areas.length === 0 ? 'Nenhuma área ainda' : `${areas.length} área${areas.length > 1 ? 's' : ''}`}
          </p>
        </div>
      ...
      {areas.length === 0 && (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm">Crie sua primeira área de estudo para começar.</p>
        </div>
      )}
          ...
          >
            ...
            <div className="flex items-center gap-3 px-5 py-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">
                  {areaConc.length} conceito{areaConc.length !== 1 ? 's' : ''}
                  {' · '}
                  {areaConc.filter((c) => c.status === 'mastered').length} dominados
                </p>
              </div>
              ...
              <div className="flex items-center gap-1">
                <button
                  title={area.isActive ? 'Arquivar área' : 'Reativar área'}
            ...
            {isOpen && (
              <div className="border-t border-gray-50 px-5 py-3 space-y-1">
                {areaConc.length === 0 ? (
                  <p className="text-xs text-gray-400 py-2">Nenhum conceito ainda.</p>
                ) : (
